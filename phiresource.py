...
def save(key, entry):
    ...
    if config["avatar"] and key[:7] == "avatar.":
    ...
    elif config["illustrationBlur"] and key[-21:-3] == "/IllustrationBlur.":
        key = key[:-21]
        bytesIO = BytesIO()
        obj.image.save(bytesIO, "png")
        queue_in.put(("illustrationBlur/%s.png" % key, bytesIO))
    elif config["illustrationLowRes"] and key[-23:-3] == "/IllustrationLowRes.":
        key = key[:-23]
        pool.submit(save_image, "illustrationLowRes/%s.png" % key, obj.image)
    elif config["illustration"] and key[-17:-3] == "/Illustration.":
        key = key[:-17]
        ...
        # queue_in.put(("music/%s.wav" % key, obj["music.wav"]))
...
def main(path):
    global c,types,config,FSB5,type_list
    c = ConfigParser()
